...
/* the addr must not be page aligned */
...
static struct faulthook_page *get_faulthook_page(unsigned long addr, pid_t pid)
{
	...
	addr = addr & page_level_mask(l);
	head = faulthook_get_page_list(addr, pid);

	/* we store the faulthook pages with page aligned addrs */
...
int faulthook_handler(struct pt_regs *regs, unsigned long addr, pid_t pid)
{
	...
	if (cleanup_faulthook_ctx[smp_processor_id()]) {
		pr_info("Cleaning ctx for cpu %d\n", smp_processor_id());
	...
	disarm_page(faultpage);
	probe = get_faulthook_probe(addr, pid);
...
int register_faulthook_probe(struct faulthook_probe *p)
{
	...
	unsigned long base_addr = p->addr & PAGE_MASK;
	...
	if (pin_to_core(p->pid, p->pin_to_core) != 0) {
		pr_warn("Pinning to cpu %d failed. Probe may not work\n",
			p->pin_to_core);
	};
...
void unregister_faulthook_probe(struct faulthook_probe *p)
{
	...
	{
		struct faulthook_page *f =
			get_faulthook_page(addr + size, p->pid);
		if (f) {
			...
			void *args)
{
	...
	if (val == DIE_DEBUG && (*dr6_p & DR_STEP))
		if (faulthook_post_handler(*dr6_p, arg->regs) == 1) {
			pr_info("faulthook_post_handler == 1, returning NOTIFY_STOP\n");
			/*
